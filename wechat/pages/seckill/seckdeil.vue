		...
		<view class="commodity">
			...
			<view v-if="barcar==1">
				<view class="scoreMove">
					...
					<view>
						<view class="attitude" v-for="(item,index) in 1" :key="index">
		...
		methods:{
			...
			async addtoCart(){
				...
				if(burIndex == null) {
					...
					if(this.shopNum>this.goodsStocknum){
						uni.showToast({
							title:'数量不能超过库存！',
							icon: 'none'
						})
			...
			async toCart(){	
				...
				this.goodsList.forEach((item,index)=>{ //判断
					if(item.goodsSpecificationId == this.goodsSpecificationId){
							...
							console.log(index)
							this.shoppingCartId = this.goodsList[index].shoppingCartId
							this.shoppingNum = this.goodsList[index].goodsNums//购物车已存数量
							this.shoppingStocknum = this.goodsList[index].list[0].goodsStock //库存
							console.log(this.shoppingCartId)
				...
				if(curIndex == null) {
					...
					this.shoppingNum++  //购物车已存数量
					if(this.shoppingNum){//购物车已存数量
						...
						console.log(this.shopNum)
						console.log(this.shoppingStocknum)
						if(this.shopNum>this.shoppingStocknum){
							uni.showToast({
								title:'数量不能超过库存！',
								icon: 'none'
							})
