<template>
	<view class="container" v-if='isDisplay'>
		<view class="inputBox" :class="change==true?'active':''" >
			<text class="cuIcon-close guan" @click="close"></text>
		  <view class="topBg">
		  	  <view class="please">
		  	  	请输入支付密码
		  	  </view>
...
<script>
	import {createCardRecord,updateCardRecord} from '../../API/membership/membership.js'
	import {
			mapState,
			mapGetters,
			mapMutations,
		   ...
		   ajaxData () {
			  ...
			  close(){
				...
				async comfirmPay() {
					...
					const res = await updateCardRecord(this.token,this.membershipCardRuleEffectiveTime,this.paymentMethod);
					uni.showToast({
						title: '支付成功',
					})
